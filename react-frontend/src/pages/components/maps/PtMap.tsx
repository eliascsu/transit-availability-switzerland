...
export default function PtMap() {
    ...
    const miau = useRef<LatLng>(new LatLng(0,0));
    ...
    function MapEvents() {
        const map = useMapEvents(
            ...
            fetch(url).then(response => response.text()).then(data => {
                ...
                console.log(data);
                if(infoPtStop.current != "" && miau.current != undefined){
                    console.log("maumau")
                    map.openPopup(infoPtStop.current, miau.current, {minWidth: 400, className: "custom-popup"});
                }
            })
            });
